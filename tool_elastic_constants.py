...
#        assert value.shape == (3,3,3,3), 'Sijkl must be 3x3x3x3'
    ...
    def tetragonal(self, **kwargs):
        """Set values with only tetragonal moduli:
    ...
    def model(self, **kwargs):
        ...
        If model is given,
        then model is converted into a DataModelDict
        and the elastic constants are read in
        if the model contains exactly one 'elastic-constants' branch.

        If model is not given,
        then a DataModelDict for the elastic constants is constructed.
        The values included will depend on the crystal system,
        nd will be converted to the specified units.
        """
        ...
        # Return DataModelDict if model not given
            ...
            for ij, value in c_dict.iteritems():
                C.append(DataModelDict([('stiffness',
                                         DataModelDict([('value', value),
                                                        ('unit', unit)])),
                                        ('ij', ij)]))
            return model
    ...
    @property
    def bulk(self):
        """The Hill bulk modulus estimate.
        Equal to average of Voigt and Reuss bulk modulus."""
        return (self.bulk_Voigt + self.bulk_Reuss) / 2

    @property
    def bulk_Voigt(self):
        """The Voigt bulk modulus estimate.
        Uses hydrostatic stress."""
        c = self.Cij
        return ((c[0, 0] + c[1, 1] + c[2, 2]) + 2 * (c[0, 1] + c[1, 2] + c[0, 2])) / 9

    @property
    def bulk_Reuss(self):
        """The Reuss bulk modulus estimate.
        Uses hydrostatic strain."""
        s = self.Sij
        return 1 / ((s[0, 0] + s[1, 1] + s[2, 2]) + 2 * (s[0, 1] + s[1, 2] + s[0, 2]))

    @property
    def shear(self):
        """The Hill shear modulus estimate.
        Equal to average of Voigt and Reuss shear modulus."""
        return (self.shear_Voigt + self.shear_Reuss) / 2

    @property
    def shear_Voigt(self):
        """The Voigt shear modulus estimate.
         Uses non-hydrostatic stresses."""
        c = self.Cij
        return ((c[0, 0] + c[1, 1] + c[2, 2]) - (c[0, 1] + c[1, 2] + c[0, 2]) + 3 * (c[3, 3] + c[4, 4] + c[5, 5])) / 15

    @property
    def shear_Reuss(self):
        """The Reuss shear modulus estimate.
         Uses non-hydrostatic strains."""
        s = self.Sij
